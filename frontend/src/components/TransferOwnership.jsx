...
const TransferOwnership = () => {
  ...
  const handleTransfer = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        setError(data.error || "Smart Contract Reverted");
      } else {
        setResult(data);
      }
    } catch (err) {
      ...
      <div className="glass-card p-6 md:p-10 mb-8 relative z-10">
        ...
        <div className="grid grid-cols-1 gap-10 mb-10 mt-4">
          <div className="space-y-3 relative group">
            ...
            <input 
              type="text" 
              placeholder="Paste the serial from the Authenticate tab..."
              className="input-field font-mono text-sm tracking-widest bg-obsidian-900/80 border-white/5 hover:border-luxury-gold/30 focus:border-luxury-gold focus:shadow-neon-gold"
              value={serial}
          ...
          <div className="space-y-3 relative group">
            ...
            <input 
              type="text" 
              className="input-field font-mono text-sm tracking-widest text-cyber-cyan bg-obsidian-900/80 border-white/5 hover:border-cyber-cyan/30 focus:border-cyber-cyan focus:shadow-neon-cyan"
              value={wallet}
